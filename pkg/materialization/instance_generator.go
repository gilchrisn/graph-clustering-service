...
nodeProblems := make(map[string][]string) // nodeID -> list of problems

for _, instance := range allInstances {
    if instance.IsValid() {
        start := instance.GetStartNode()
        instancesPerNode[start]++
    }
}
...
if len(ig.metaPath.EdgeSequence) > 0 {
    requiredFirstEdge = ig.metaPath.EdgeSequence[0]
    requiredFirstTarget = ig.metaPath.NodeSequence[1]
}

var zeroInstanceNodes []string
for nodeID, node := range ig.graph.Nodes {
    if node.Type == startNodeType && instancesPerNode[nodeID] == 0 {
        zeroInstanceNodes = append(zeroInstanceNodes, nodeID)
        
        // Diagnose why this node has zero instances
        problems := []string{}
        outgoingEdges := ig.edgesByFrom[nodeID]
        
        if len(outgoingEdges) == 0 {
            problems = append(problems, "ISOLATED_NODE")
        } else {
            hasCorrectEdgeType := false
            hasCorrectTarget := false
            
            for _, edge := range outgoingEdges {
                if edge.Type == requiredFirstEdge {
                    hasCorrectEdgeType = true
                    if targetNode, exists := ig.graph.Nodes[edge.To]; exists {
                        if targetNode.Type == requiredFirstTarget {
                            hasCorrectTarget = true
                            // This should work, investigate further
                            problems = append(problems, "SHOULD_WORK_BUT_DOESNT")
                            break
                        }
                    }
                }
            ...
            if !hasCorrectEdgeType {
                problems = append(problems, "WRONG_EDGE_TYPE")
            }
            if hasCorrectEdgeType && !hasCorrectTarget {
                problems = append(problems, "WRONG_TARGET_TYPE")
            }
        }
        
        nodeProblems[nodeID] = problems
    }
}

fmt.Printf("🚨 STAGE 1: %d nodes produced 0 instances: %v\n", len(zeroInstanceNodes), zeroInstanceNodes)
fmt.Printf("📊 STAGE 1 BREAKDOWN:\n")
...
for nodeID, problems := range nodeProblems {
    fmt.Printf("   Node %s: %v\n", nodeID, problems)
    for _, problem := range problems {
        switch problem {
        case "ISOLATED_NODE":
            isolated++
        case "WRONG_EDGE_TYPE":
            wrongEdgeType++
        case "WRONG_TARGET_TYPE":
            wrongTargetType++
        case "SHOULD_WORK_BUT_DOESNT":
            shouldWork++
        }
    }
}

fmt.Printf("   📈 Summary: %d isolated, %d wrong edge type, %d wrong target type, %d should work\n", 
    isolated, wrongEdgeType, wrongTargetType, shouldWork)

// If any "should work" nodes, investigate deeper
if shouldWork > 0 {
    fmt.Printf("🔍 INVESTIGATING 'SHOULD WORK' NODES:\n")
    for nodeID, problems := range nodeProblems {
        for _, problem := range problems {
            if problem == "SHOULD_WORK_BUT_DOESNT" {
                fmt.Printf("   🧐 Node %s analysis:\n", nodeID)
                
                // Check if this node appears in any worker's queue
                fmt.Printf("      - Checking if node was processed by workers...\n")
                
                // Manually trace path from this node
                instances := ig.findInstancesFromNode(context.Background(), nodeID)
                fmt.Printf("      - Manual trace found %d instances\n", len(instances))
                
                if len(instances) == 0 {
                    fmt.Printf("      - Manual trace also found 0 instances - investigating BFS logic\n")
                    // Add more detailed BFS tracing here if needed
                }
            }
        }
    }
