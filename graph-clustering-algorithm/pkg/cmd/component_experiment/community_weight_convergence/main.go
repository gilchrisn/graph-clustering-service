...
// Configuration for community weight distribution experiment
...
// CommunityWeightDistributionSample captures community weight calculations at a specific move
type CommunityWeightDistributionSample struct {
	MoveID                int                            `json:"move_id"`
	Node                  int                            `json:"node"`
	CurrentCommunity      int                            `json:"current_community"`
	NumCommunities        int                            `json:"num_communities"`
	LouvainCommunityWeights []float64                    `json:"louvain_community_weights"`   // Weight of each community
	ScarCommunityWeights    map[int][]float64            `json:"scar_community_weights"`      // k -> community weights
...
// CommunityWeightDistributionExperiment contains all experiment data
type CommunityWeightDistributionExperiment struct {
	Config             ExperimentConfig             `json:"config"`
	GraphInfo          GraphInfo                    `json:"graph_info"`
	Samples            []CommunityWeightDistributionSample `json:"samples"`
...
func main() {
	...
	fmt.Println("🎯 COMMUNITY WEIGHT DISTRIBUTION ANALYSIS")
	fmt.Println("==========================================")
	fmt.Println("Comparing community weight estimation distributions: Louvain vs SCAR(k)")

	// Experiment configuration
	expConfig := ExperimentConfig{
		SampleInterval:  1,  // Sample every move
		KValues:        []int{2, 8, 32, 64, 128, 256, 512, 1024},
	...
	// Step 4: Run community weight distribution experiment
	fmt.Println("\nStep 4: Running community weight distribution experiment...")
	experiment := runCommunityWeightDistributionExperiment(materializedGraph, louvainMoves, scarCache, expConfig)

	// Step 5: Export results
	fmt.Println("\nStep 5: Exporting results...")
	if err := exportCommunityWeightDistributionExperiment(experiment); err != nil {
...
func runCommunityWeightDistributionExperiment(graph *louvain.Graph, moves []utils.MoveEvent, 
	scarCache *ScarGraphCache, config ExperimentConfig) *CommunityWeightDistributionExperiment {
	
	experiment := &CommunityWeightDistributionExperiment{
		Config: config,
		GraphInfo: GraphInfo{
		...
		Samples:           make([]CommunityWeightDistributionSample, 0),
	...
	for moveIdx, move := range moves {
		...
		// Sample community weight distributions at specified intervals
		if moveIdx%config.SampleInterval == 0 && sampleCount < config.MaxSamples {
			if moveIdx+1 < len(moves) { // Look ahead to next move
				...
				sample := captureCommunityWeightDistribution(graph, nextMove.Node, louvainComm, scarCache, config.KValues)
...
func captureCommunityWeightDistribution(graph *louvain.Graph, node int, louvainComm *louvain.Community,
	scarCache *ScarGraphCache, kValues []int) CommunityWeightDistributionSample {
	
	sample := CommunityWeightDistributionSample{
		Node:                    node,
		CurrentCommunity:        louvainComm.NodeToCommunity[node],
		NumCommunities:          louvainComm.NumCommunities,
		ScarCommunityWeights:    make(map[int][]float64),
	...
	// Calculate SCAR community weights for each k value
	for _, k := range kValues {
		...
		if node < scarGraph.NumNodes {
			...
			if len(commonCommunities) > 0 {
				// Calculate weights for common communities only
				sample.LouvainCommunityWeights = calculateWeightsForCommunities(graph, louvainComm, commonCommunities)
				sample.ScarCommunityWeights[k] = calculateScarWeightsForCommunities(scarGraph, scarComm, commonCommunities)
				
				// Now both arrays have the same communities in the same order
				sample.DistanceMeasures[k] = calculateDistanceMeasures(sample.LouvainCommunityWeights, sample.ScarCommunityWeights[k])
...
// Calculate weights for specific community IDs (in order)
func calculateWeightsForCommunities(graph *louvain.Graph, comm *louvain.Community, communityIDs []int) []float64 {
	weights := make([]float64, len(communityIDs))
	for i, commID := range communityIDs {
		weights[i] = comm.CommunityWeights[commID]
	}
	return weights
}

// Calculate SCAR weights for specific community IDs (in order)
func calculateScarWeightsForCommunities(graph *scar.SketchGraph, comm *scar.Community, communityIDs []int) []float64 {
	weights := make([]float64, len(communityIDs))
	for i, commID := range communityIDs {
		weights[i] = graph.EstimateCommunityCardinality(commID, comm)
	}
	return weights
}

// Calculate weights of ALL communities (Louvain exact method)
func calculateAllCommunityWeights(graph *louvain.Graph, comm *louvain.Community) []float64 {
	activeCommunityWeights := make([]float64, 0, comm.NumCommunities)
	
	for commID := 0; commID < comm.NumCommunities; commID++ {
		if len(comm.CommunityNodes[commID]) > 0 {  // Only include non-empty communities
			activeCommunityWeights = append(activeCommunityWeights, comm.CommunityWeights[commID])
		}
	}
	
	return activeCommunityWeights
}

// Calculate SCAR community weights for all communities (sketch-based method)
func calculateScarAllCommunityWeights(graph *scar.SketchGraph, comm *scar.Community) []float64 {
	activeCommunityWeights := make([]float64, 0, comm.NumCommunities)
	
	for commID := 0; commID < comm.NumCommunities; commID++ {
		if len(comm.CommunityNodes[commID]) > 0 {  // Only include non-empty communities
			// Use SCAR's sketch-based community weight estimation
			communityWeight := graph.EstimateCommunityCardinality(commID, comm)
			activeCommunityWeights = append(activeCommunityWeights, communityWeight)
		}
	}
...
func calculateAggregatedMetrics(samples []CommunityWeightDistributionSample, kValues []int) map[int]AggregatedMetrics {
...
func exportCommunityWeightDistributionExperiment(experiment *CommunityWeightDistributionExperiment) error {
	// Export main experiment data
	file, err := os.Create("community_weight_distribution_experiment.json")
	if err != nil {
		return fmt.Errorf("failed to create community_weight_distribution_experiment.json: %w", err)
	}
	defer file.Close()
	...
	for _, metric := range []string{"mae", "wasserstein_p1", "cosine_similarity"} {
		...
		filename := fmt.Sprintf("community_weight_distance_table_%s.json", metric)
	...
	fmt.Println("✅ Community weight distribution experiment exported to community_weight_distribution_experiment.json")
	return nil
}
...
func createDistanceMetricTable(experiment *CommunityWeightDistributionExperiment, metricName string) *DistanceMetricTable {
...
func displayExperimentSummary(experiment *CommunityWeightDistributionExperiment) {
	fmt.Println("\n📊 COMMUNITY WEIGHT DISTRIBUTION EXPERIMENT SUMMARY")
	fmt.Println("====================================================")
	...
	fmt.Println("\nAVERAGE COMMUNITY WEIGHT DISTANCE MEASURES:")
	...
	fmt.Println("\n💡 INTERPRETATION:")
	fmt.Println("   • Lower MAE/Wasserstein = more similar community weight distributions")
	fmt.Println("   • Higher Cosine similarity = more similar community weight distributions")
	fmt.Println("   • As k → ∞, expect SCAR community weight estimates → Louvain exact community weights")
	fmt.Println("   • This measures how well SCAR's sketch-based community weight estimation works")
}
