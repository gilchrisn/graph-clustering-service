...
// Configuration for gain distribution experiment
type ExperimentConfig struct {
	SampleInterval     int      `json:"sample_interval"`     // Sample every N moves
	KValues           []int    `json:"k_values"`            // SCAR k values to test
	DistanceMetrics   []string `json:"distance_metrics"`    // ["kl", "jensen", "wasserstein"]
	MaxSamples        int      `json:"max_samples"`         // Maximum number of samples to collect
}

// GainDistributionSample captures gain calculations at a specific move
type GainDistributionSample struct {
	MoveID              int                            `json:"move_id"`
	Node                int                            `json:"node"`
	CurrentCommunity    int                            `json:"current_community"`
	NumCommunities      int                            `json:"num_communities"`
	LouvainGains        []float64                      `json:"louvain_gains"`
	ScarGains           map[int][]float64              `json:"scar_gains"`           // k -> gains
...
// GainDistributionExperiment contains all experiment data
type GainDistributionExperiment struct {
	Config             ExperimentConfig             `json:"config"`
	GraphInfo          GraphInfo                    `json:"graph_info"`
	Samples            []GainDistributionSample     `json:"samples"`
...
type AggregatedMetrics struct {
	...
	AvgJensenShannon    float64 `json:"avg_jensen_shannon"`
	...
	StdKLDivergence     float64 `json:"std_kl_divergence"`
	StdJensenShannon    float64 `json:"std_jensen_shannon"`
...
func main() {
	...
	fmt.Println("🎯 GAIN DISTRIBUTION ANALYSIS")
	fmt.Println("============================")
	fmt.Println("Comparing gain calculation distributions: Louvain vs SCAR(k)")

	// Experiment configuration
	expConfig := ExperimentConfig{
		SampleInterval:  10,  // Sample every 5 moves
		KValues:        []int{2, 16, 64, 128, 256, 512, 1024},
		DistanceMetrics: []string{"kl", "jensen", "wasserstein"},
		MaxSamples:     3500, // Limit samples for manageable output
	}
	...
	// Step 4: Run gain distribution experiment
	fmt.Println("\nStep 4: Running gain distribution experiment...")
	experiment := runGainDistributionExperiment(materializedGraph, louvainMoves, scarCache, expConfig)

	// Step 5: Export results
	fmt.Println("\nStep 5: Exporting results...")
	if err := exportGainDistributionExperiment(experiment); err != nil {
...
func runGainDistributionExperiment(graph *louvain.Graph, moves []utils.MoveEvent, 
	scarCache *ScarGraphCache, config ExperimentConfig) *GainDistributionExperiment {
	
	experiment := &GainDistributionExperiment{
		Config: config,
		GraphInfo: GraphInfo{
		...
		Samples:           make([]GainDistributionSample, 0),
	...
	for moveIdx, move := range moves {
		...
		// Sample gain distributions at specified intervals
		if moveIdx%config.SampleInterval == 0 && sampleCount < config.MaxSamples {
			if moveIdx+1 < len(moves) { // Look ahead to next move
				...
				sample := captureGainDistribution(graph, nextMove.Node, louvainComm, scarCache, config.KValues)
...
func captureGainDistribution(graph *louvain.Graph, node int, louvainComm *louvain.Community,
	scarCache *ScarGraphCache, kValues []int) GainDistributionSample {
	
	sample := GainDistributionSample{
		Node:             node,
		CurrentCommunity: louvainComm.NodeToCommunity[node],
		NumCommunities:   louvainComm.NumCommunities,
		ScarGains:        make(map[int][]float64),
		DistanceMeasures: make(map[int]DistanceMeasures),
	}

	// Calculate Louvain gains to ALL communities
	sample.LouvainGains = calculateAllCommunityGains(graph, louvainComm, node)

	// Calculate SCAR gains for each k value (direct node mapping)
	for _, k := range kValues {
		...
		if node < scarGraph.NumNodes {
			scarGains := calculateScarAllCommunityGains(scarGraph, scarComm, node)
			sample.ScarGains[k] = scarGains
			
			// Calculate distance measures
			sample.DistanceMeasures[k] = calculateDistanceMeasures(sample.LouvainGains, scarGains)
		}
	}
...
// Calculate modularity gains to ALL communities (extended Louvain logic)
func calculateAllCommunityGains(graph *louvain.Graph, comm *louvain.Community, node int) []float64 {
	gains := make([]float64, comm.NumCommunities)
	
	for targetComm := 0; targetComm < comm.NumCommunities; targetComm++ {
		if len(comm.CommunityNodes[targetComm]) == 0 {
			gains[targetComm] = 0.0
			continue
		}
		...
		gains[targetComm] = louvain.CalculateModularityGain(graph, comm, node, targetComm, edgeWeight)
	}
	
	return gains
}

// Calculate SCAR gains to all communities
func calculateScarAllCommunityGains(graph *scar.SketchGraph, comm *scar.Community, node int) []float64 {
	gains := make([]float64, comm.NumCommunities)
	...
	for targetComm := 0; targetComm < comm.NumCommunities; targetComm++ {
		...
		gains[targetComm] = scar.CalculateModularityGain(graph, comm, node, targetComm, edgeWeight)
	}
...
func calculateDistanceMeasures(p, q []float64) DistanceMeasures {
	// Normalize distributions to make them proper probability distributions
	pNorm := normalize(p)
	qNorm := normalize(q)
	
	return DistanceMeasures{
		KLDivergence:       klDivergence(pNorm, qNorm),
		JensenShannon:      jensenShannonDivergence(pNorm, qNorm),
		WassersteinP1:      wassersteinP1(pNorm, qNorm),
...
func wassersteinP1(p, q []float64) float64 {
	...
	cdfP := make([]float64, len(p))
	cdfQ := make([]float64, len(q))
...
func calculateAggregatedMetrics(samples []GainDistributionSample, kValues []int) map[int]AggregatedMetrics {
	aggregated := make(map[int]AggregatedMetrics)
	
	for _, k := range kValues {
		metrics := AggregatedMetrics{K: k}
		
		klValues := make([]float64, 0)
		jsValues := make([]float64, 0)
		...
		for _, sample := range samples {
			if measures, exists := sample.DistanceMeasures[k]; exists {
				if !math.IsInf(measures.KLDivergence, 1) && !math.IsNaN(measures.KLDivergence) {
					klValues = append(klValues, measures.KLDivergence)
				}
				if !math.IsInf(measures.JensenShannon, 1) && !math.IsNaN(measures.JensenShannon) {
					jsValues = append(jsValues, measures.JensenShannon)
				}
		...
		metrics.NumSamples = len(klValues)
		metrics.AvgKLDivergence = average(klValues)
		metrics.AvgJensenShannon = average(jsValues)
		...
		metrics.StdKLDivergence = standardDeviation(klValues)
		metrics.StdJensenShannon = standardDeviation(jsValues)
...
func exportGainDistributionExperiment(experiment *GainDistributionExperiment) error {
	// Export main experiment data
	file, err := os.Create("gain_distribution_experiment.json")
	if err != nil {
		return fmt.Errorf("failed to create gain_distribution_experiment.json: %w", err)
	}
	defer file.Close()
	...
	for _, metric := range []string{"kl_divergence", "jensen_shannon", "wasserstein_p1", "cosine_similarity"} {
		table := createDistanceMetricTable(experiment, metric)
		filename := fmt.Sprintf("distance_table_%s.json", metric)
	...
	fmt.Println("✅ Gain distribution experiment exported to gain_distribution_experiment.json")
	return nil
}
...
func createDistanceMetricTable(experiment *GainDistributionExperiment, metricName string) *DistanceMetricTable {
	...
	for kIdx, k := range experiment.Config.KValues {
		...
		for sampleIdx, sample := range experiment.Samples {
			...
			if measures, exists := sample.DistanceMeasures[k]; exists {
				switch metricName {
				case "kl_divergence":
					distance = measures.KLDivergence
				case "jensen_shannon":
					distance = measures.JensenShannon
				case "wasserstein_p1":
...
func displayExperimentSummary(experiment *GainDistributionExperiment) {
	fmt.Println("\n📊 GAIN DISTRIBUTION EXPERIMENT SUMMARY")
	fmt.Println("=======================================")
	...
	for _, metric := range []string{"kl_divergence", "jensen_shannon"} {
			...
			for i := 0; i < sampleCount; i++ {
				if i < len(experiment.Samples) {
					...
					if measures, exists := sample.DistanceMeasures[k]; exists {
						...
						switch metric {
						case "kl_divergence":
							value = measures.KLDivergence
						case "jensen_shannon":
							value = measures.JensenShannon
						}
	...
	fmt.Printf("%-8s %-12s %-12s %-12s %-12s\n", "K", "KL-Div", "Jensen-S", "Wasserst", "Cosine")
	fmt.Println("---------------------------------------------------------------")
	...
	for _, k := range sortedKs {
		...
		fmt.Printf("%-8d %-12.6f %-12.6f %-12.6f %-12.6f\n", 
			k, metrics.AvgKLDivergence, metrics.AvgJensenShannon, 
			metrics.AvgWassersteinP1, metrics.AvgCosineSimilarity)
	}
	
	fmt.Println("\n💡 INTERPRETATION:")
	fmt.Println("   • Lower KL/Jensen-Shannon/Wasserstein = more similar distributions")
	fmt.Println("   • Higher Cosine similarity = more similar distributions")
	fmt.Println("   • As k → ∞, expect SCAR distributions → Louvain distributions")
	fmt.Println("   • If convergence fails, SCAR's sketch approximation is the cause")
}
